//These classes allow for the creation of agent popuations usinga variety of different methods
#include "agent.h"
...
static agentFactory&  select(std::string name){
    ...
    if (F==nullptr)cout<<"Name "<<name<<" not regonised in agentFactorySelector"<<endl;
    assert(F!=nullptr);
    return *F;
    }
};
